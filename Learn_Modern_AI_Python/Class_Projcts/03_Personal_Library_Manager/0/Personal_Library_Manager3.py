...
def display_book_details(book):
    st.text_input("Title", value=book.get('title', ''), key="detail_title")
    st.text_input("Author", value=book.get('author', ''), key="detail_author")
    st.number_input("Year", min_value=-1000, max_value=2100, value=book.get('year', 0), key="detail_year")
    st.text_input("Genre", value=book.get('genre', ''), key="detail_genre")
    st.checkbox("Mark as Read", value=book.get('read', False), key="detail_read")
    st.text_input("Link", value=book.get('link', ''), key="detail_link")
    if st.button("Save Changes", key="save_changes"):
        book['title'] = st.session_state.detail_title.strip()
        book['author'] = st.session_state.detail_author.strip()
        book['year'] = int(st.session_state.detail_year)
        book['genre'] = st.session_state.detail_genre.strip()
        book['read'] = st.session_state.detail_read
        book['link'] = st.session_state.detail_link.strip()
...
def main():
    ...
    st.markdown("Manage your reading collection with ease — add, view details, remove, reset, and track your books.")
    ...
    if menu == "Add Book":
        ...
        if not df.empty:
            ...
            # Select book to view details
            st.subheader("🔖 Select a book to view/edit details")
            selected = st.selectbox("", df_display['title'].tolist(), key="view_select")
            book = next((b for b in st.session_state.library if b.get('title') == selected), None)
            if book:
        ...
        if keyword:
            ...
            if results:
                ...
                df_res_display["read"] = df_res_display["read"].apply(lambda x: "✅" if x else "❌")
                df_res_display["link"] = df_res_display["link"].apply(lambda x: f"[Link]({x})" if x else "")
                st.dataframe(df_res_display, use_container_width=True)
                # Select a book from results
                st.subheader("🔖 Select a book from results to view/edit details")
                selected = st.selectbox("", df_res_display['title'].tolist(), key="search_select")
                book = next((b for b in st.session_state.library if b.get('title') == selected), None)
                if book:
                    display_book_details(book)
            else:
