...
if view == "Add Book":
    ...
    if not st.session_state.library:
        ...
        for i, book in enumerate(st.session_state.library):
            ...
            st.markdown(f"""
            <div class="book-card">
                <strong>{book['title']}</strong> by {book['author']} ({book['publication_year']})<br>
                <em>{book['genre']}</em><br>
                Status: <span class="{ 'read-badge' if book['read_status'] else 'unread-badge' }">{status}</span>
                <br><small>Added on: {book['added_date']}</small>
            </div>
            """, unsafe_allow_html=True)
