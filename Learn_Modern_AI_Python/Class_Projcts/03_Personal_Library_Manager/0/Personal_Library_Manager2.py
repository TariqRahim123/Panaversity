...
def main():
    ...
    menu = st.sidebar.radio("Menu", ["Add Book", "View Library", "Book Details", "Search", "Statistics", "Remove Book", "Reset Library"]);
    ...
    if menu == "Add Book":
        ...
        if titles:
            selected = st.selectbox("Select a book to view/edit", titles)
            ...
            if book:
                ...
                read_status = st.checkbox("Mark as Read", value=book.get('read', False), key="detail_read")
                st.text_input("Link", value=book.get('link', ''), key="detail_link")

                if st.button("Save Changes"):
                    # Update book in session_state
