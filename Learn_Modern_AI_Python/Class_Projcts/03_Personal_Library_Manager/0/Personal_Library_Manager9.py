...
if view == "Add Book":
    ...
    fig, ax = plt.subplots()
    sns.barplot(x="Count", y="Genre", data=df_genre, ax=ax)
    st.pyplot(fig)

    st.subheader("📊 Author Distribution")
    fig, ax = plt.subplots()
    sns.barplot(x="Count", y="Author", data=df_author.head(10), ax=ax)  # limit to top 10
    st.pyplot(fig)

    st.subheader("📊 Books by Decade")
    fig, ax = plt.subplots()
    sns.barplot(x="Decade", y="Count", data=df_decade, ax=ax)
    st.pyplot(fig)
